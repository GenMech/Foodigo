...
const Cart = (props) => {
  ...
  const submitOrderHandler = async (userData) => {
    ...
    await fetch(
      "https://foodigoo-b5728-default-rtdb.asia-southeast1.firebasedatabase.app/order.json",
      {
        method: "POST",
        body: JSON.stringify({
          user: userData,
  ...
  const cartModalContent = (
    // in jsx no sibling code allowed so we use fragments
    <React.Fragment>
      {cartItems}
      {/* Total amount */}
  ...
  return (
    // we have to show this up in a the modal wrapper rather than div
